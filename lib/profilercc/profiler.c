...
void disable_hooks(void) {
  if (hooks_enabled) {
    profile__hook_redo=profile__hook_nop;
    profile__hook_cut=profile__hook_nop;
    profile__hook_call=profile__hook_call_nop;
# if defined(PROFILE__TRACER)
    profile__hook_fail=profile__hook_nop;
    profile__hook_proceed=profile__hook_nop;
# endif
    hooks_enabled=FALSE;
  }
}
...
const char *predicate_type(int t); /* defined in eng_profile.c */
