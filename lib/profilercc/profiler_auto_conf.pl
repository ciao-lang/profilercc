...
:- doc(module, "This module performs an automatic cost center
   configuration for a given goal. As a result it gives the trace of
   the highest resource (e.g., time, counts, ticks, etc.) consuming
   path starting from the goal.").
...
:- use_module(library(hrtime/profiler_extra)).
:- use_module(library(profilercc/profiler_type)).
...
cc_auto_conf_(Field, Modules, Module, Goal, Cond, Goals, Tree, ProfileInfo,
    ...
    glbmod_add_package(Modules, profilercc),
    maplist(write_all_cc_decl, Modules),
...
cc_auto_conf_iter(Field, Goal, Modules, PredName0, EdgesCG, FN, Cond, Goals0,
    ...
    maplist(goal_to_cc, Goals0, CCGoals0),
    ...
    foldl(cc_to_goal, MaxCCs, Goals1, Goals0),
...
:- use_module(ciaobld(ciaoc_aux), [clean_mods/1]).

:- pred clean_modules/1 : list(atm)
# "Remove the files auto generated by the compiler (itf, po) in order
    to force recompilation.".

clean_modules(Modules) :-
    maplist(rev_defines_module, Modules, Bases),
    clean_mods(Bases).

rev_defines_module(Mod,Base) :- defines_module(Base,Mod). % TODO: good indexing?
...
tree_to_tex(NodeField, g(Nodes, Edges)) :-
    ...
    maplist(adapt_graph_edge, FlatEdges, EdgesG),
    maplist(adapt_graph_node, Nodes, NodesG),
